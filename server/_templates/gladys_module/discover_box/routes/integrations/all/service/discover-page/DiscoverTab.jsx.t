...
class DiscoverTab extends Component {
  async componentWillMount() {
    ...
    this.props.session.dispatcher.addListener(WEBSOCKET_MESSAGE_TYPES.<%= constName %>.NEW_DEVICE, this.addDiscoveredDevice);
  }

  componentWillUnmount() {
    this.props.session.dispatcher.removeListener(
      WEBSOCKET_MESSAGE_TYPES.<%= constName %>.NEW_DEVICE,
  ...
  searchDevices = async () => {
    ...
    try {
      await this.props.httpClient.post(`/api/v1/service/<%= module %>/discover`);
      this.setState({
        discoveredDevices: [],
        errorLoading: false
      });
  ...
  getDiscoveredDevices = async () => {
    ...
    try {
      const discoveredDevices = await this.props.httpClient.get(`/api/v1/service/<%= module %>/discover`);
      this.setState({
        discoveredDevices,
        loading: false,
        errorLoading: false
